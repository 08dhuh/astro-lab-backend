...
from app.services.db_service import check_database_status, get_all_clusters, get_isochrones, get_cluster_ubv,get_all_cluster_UBVS
...
@router.get("/debug/clusters/{cluster_id}")
def api_get_cluster_ubv(cluster_id: int):
    ubv_data = get_cluster_ubv(cluster_id)
    if ubv_data is None:
        raise HTTPException(status_code=404, detail=f"Cluster not found for {cluster_id}")
    return ubv_data